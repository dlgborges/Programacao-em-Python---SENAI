...
users = {}
users['username_1'] = input('Digite o nome do usuário a cadastrar: ')
users['username_1_password'] = input('Digite a senha do usuário: ')

print(f'Usuário {users['username_1']} cadastrado!\n\nSistema de Notas dos Alunos - Visão do Professor')

login_user = input('Digite o nome do usuário a fazer login: ')
login_user_password = input('Digite a senha do usuário a fazer login: ')
chances = 3

for n in range(chances):
    print('Você tem: ', chances, 'chances.')
    if (login_user in users.values()) and (login_user_password == users['username_1_password']):
        print('Bem-vindo ao sistema de Notas dos Alunos - Visão do Professor\n')

        # - Inserir notas (se Senha correta)
        print('\nAgora, vamos inserir as notas dos alunos no sistema:')

        more_grades = input('\nDigite SAIR quando não tiver mais notas a inserir OU pressione qualquer tecla para continuar. ')
        students_grades = []
